...
import { Sparkles, TrendingUp, Calendar, Palette, Repeat, BarChart3, Link2, Mic, Rocket, Clapperboard, Megaphone, ShoppingBag, Building2 } from "lucide-react";
...
function FeatureCard({ icon: Icon, title, desc, accent }: { icon: React.ElementType; title: string; desc: string; accent: string }) {
  return (
    <div className="group relative rounded-2xl p-6 border border-white/10 bg-white/5 hover:bg-white/10 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl backdrop-blur-sm overflow-hidden">
      ...
      <div className="w-12 h-12 rounded-xl flex items-center justify-center mb-4" style={{ background: accent + "22", border: `1px solid ${accent}44` }}>
...
export default function Home() {
  ...
  const features = [
    { icon: Sparkles, title: "AI Caption Generator", desc: "Generate platform-optimized captions for Instagram, TikTok, LinkedIn and X in seconds, tailored to your brand voice.", accent: "#6C5CE7" },
    { icon: TrendingUp, title: "Trend Intelligence", desc: "Real-time trend monitoring across all major platforms. Always be first to ride a wave before it peaks.", accent: "#00b894" },
    { icon: Calendar, title: "Content Calendar", desc: "AI-powered scheduling fills your entire month with tailored content ideas — automatically, no thinking required.", accent: "#fd79a8" },
    { icon: Palette, title: "Visual Assets", desc: "Generate on-brand graphics, carousels, and story templates that match your aesthetic in one click.", accent: "#fdcb6e" },
    { icon: Repeat, title: "Auto-Repurpose", desc: "Turn one blog post into 10 pieces of content. Automatically repurpose long-form content for every platform.", accent: "#74b9ff" },
    { icon: BarChart3, title: "Analytics Dashboard", desc: "Track what's working, what's not, and get AI recommendations to double down on your best-performing content.", accent: "#a78bfa" },
  ];
    ...
    <div style={{ fontFamily: "'Sora',sans-serif", background: "linear-gradient(160deg,#0d0820 0%,#110d2e 40%,#0a1a18 100%)", minHeight: "100vh", color: "white" }}>
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-black/40 backdrop-blur-xl border-b border-white/10" : "bg-transparent"}`}>
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            <Link href="/login" className="text-white/60 hover:text-white text-sm transition-colors px-3 py-1.5">Sign in</Link>
            <Link href="/register" className="bg-linear-to-r from-violet-500 to-purple-600 text-white text-sm font-semibold px-5 py-2 rounded-xl hover:-translate-y-px hover:shadow-lg hover:shadow-violet-500/30 transition-all duration-200">Start Free</Link>
          </div>
      ...
      <section className="py-24 px-6">
        ...
        <div className="max-w-4xl mx-auto grid md:grid-cols-3 gap-8">
          {[
            { step: "01", title: "Connect your profiles", desc: "Link your Instagram, TikTok, LinkedIn and other social accounts in one click.", icon: Link2 },
            { step: "02", title: "Describe your brand", desc: "Tell us your niche, tone, and audience. CreatorAI learns your voice in minutes.", icon: Mic },
            { step: "03", title: "Generate & schedule", desc: "Get AI-powered content ideas, captions, and a full calendar — ready to post.", icon: Rocket },
          ].map((s, i) => (
            <div key={i} className="relative rounded-2xl p-6 border border-white/10 bg-white/5 backdrop-blur-sm text-center">
              <div className="mb-4 flex justify-center"><s.icon size={32} className="text-violet-300" strokeWidth={1.75} /></div>
      ...
      <section className="py-20 px-6">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { title: "Content Creators", desc: "Batch a month of content in one afternoon.", icon: Clapperboard, color: "#a78bfa" },
              { title: "Marketers", desc: "Launch campaigns faster and always stay on-trend.", icon: Megaphone, color: "#00b894" },
              { title: "eCommerce Brands", desc: "Generate product captions that actually convert.", icon: ShoppingBag, color: "#fd79a8" },
              { title: "Agencies", desc: "Manage multiple client profiles from one dashboard.", icon: Building2, color: "#fdcb6e" },
            ].map((u, i) => (
              <div key={i} className="rounded-2xl p-5 border border-white/10 bg-white/5 backdrop-blur-sm hover:bg-white/8 transition-all duration-200">
                <div className="mb-3"><u.icon size={26} style={{ color: u.color }} strokeWidth={1.75} /></div>
      ...
      <section className="py-24 px-6">
        <div className="max-w-3xl mx-auto text-center relative">
          ...
          <div className="relative rounded-3xl border border-white/10 bg-white/5 backdrop-blur-sm p-14">
            <div className="mb-5 flex justify-center"><Rocket size={44} className="text-violet-300" strokeWidth={1.5} /></div>
