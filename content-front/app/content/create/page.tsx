...
/* ══ DATA ══════════════════════════════════════════════════════ */
const POST_TYPES = [
  { id: "blog", label: "Blog Post", Icon: PenLine, color: "#a78bfa", desc: "Long-form article with SEO structure" },
  { id: "social", label: "Social Media", Icon: Radio, color: "#fd79a8", desc: "Platform-native posts with hashtags" },
  { id: "product", label: "Product Drop", Icon: ShoppingBag, color: "#fbbf24", desc: "Sales copy that converts" },
  { id: "youtube", label: "YouTube", Icon: Play, color: "#f87171", desc: "Titles, descriptions & tags" },
  { id: "tiktok", label: "TikTok", Icon: Music2, color: "#34d399", desc: "Hooks, captions & viral angles" },
];
...
const TONES = [
  ...
  { id: "educational", label: "Educate", Icon: BookOpen },
  { id: "persuasive", label: "Persuade", Icon: Target },
];
...
function Particles({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    ...
    const list = Array.from({ length: 24 }, (_, i) => ({
      ...
      size: 2 + Math.random() * 3, color: ["#6C5CE7", "#22d3ee", "#fd79a8", "#fbbf24"][i % 4],
...
export default function ContentStudio() {
  ...
  const generate = async () => {
    ...
    try {
      ...
      // Normalize response shape: social returns an array (one per platform),
      // blog/product returns a single object
      const isMulti = Array.isArray(data);
      const contentId = isMulti ? (data[0]?.id ?? crypto.randomUUID()) : data?.id;
      ...
      if (typeof window !== "undefined" && contentId) {
        const posts = isMulti
          ...
          : [{ platform: null, content: data.body ?? data.content, seo_report: data.seo_report }];
  ...
  return (
    <div className="min-h-screen text-white overflow-x-hidden"
      ...
      <div className="relative z-5 max-w-215 mx-auto px-6 py-12 pb-28">
        ...
        <div className="relative mb-16">
          ...
          <div className="relative">
            ...
            <div className="absolute -inset-px rounded-[20px] pointer-events-none transition-opacity duration-500"
              style={{
                background: "linear-gradient(135deg,rgba(108,92,231,.6),rgba(34,211,238,.35),rgba(253,121,168,.2))",
        ...
        <div className="mb-16">
          ...
          <div className="relative h-47 flex items-center justify-center select-none">
            ...
            <button onClick={prev}
              ...
              <prevType.Icon className="w-7 h-7 text-white/70" />
              <span className="text-white/55 text-xs font-semibold">{prevType.label}</span>
            </button>
            ...
            <div className="card-float relative flex flex-col items-center gap-2 text-center rounded-[22px] py-5 px-5 z-10 w-43"
              style={{
                background: `linear-gradient(145deg,${at.color}22,rgba(0,0,0,.6))`,
              ...
              <at.Icon className="w-9 h-9" style={{ color: at.color }} strokeWidth={1.75} />
            ...
            <button onClick={next}
              ...
              style={{ transform: "scale(.83) translateY(10px)", opacity: .4 }}>
              <nextType.Icon className="w-7 h-7 text-white/70" />
              <span className="text-white/55 text-xs font-semibold">{nextType.label}</span>
            </button>
        ...
        {postType === "social" && (
          <div className="mb-16">
            <StepLabel num="3" label="Broadcast to" c1="#22d3ee" c2="#0984e3" />
              ...
              {PLATFORMS.map(p => {
                ...
                return (
                  <button key={p.id} onClick={() => toggle(p.id)}
                    ...
                    style={{
                      background: on ? `${p.color}18` : "rgba(255,255,255,.03)",
            ...
            <div className="grid grid-cols-3 gap-2">
              {TONES.map(t => (
                <button key={t.id} onClick={() => setTone(t.id)}
                  ...
                  style={{
                    background: tone === t.id ? "rgba(167,139,250,.17)" : "rgba(255,255,255,.03)",
                    borderColor: tone === t.id ? "rgba(167,139,250,.65)" : "rgba(255,255,255,.07)",
                    boxShadow: tone === t.id ? "0 0 18px rgba(167,139,250,.22)" : "none"
                  }}>
                  <t.Icon className="w-5 h-5" style={{ color: tone === t.id ? "#c4b5fd" : "rgba(255,255,255,.45)" }} strokeWidth={1.75} />
